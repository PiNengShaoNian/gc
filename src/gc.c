 ...
 * free the the memory pointed to by `a->ptr`.
 *
...
static void *gc_allocate(GarbageCollector *gc, size_t count, size_t size, void (*dtor)(void *))
{
    ...
    /* If allocation fails, force an out-f-policy run to free some memory and try again. */
...
void gc_start_ext(GarbageCollector *gc, void *bos, size_t initial_capacity, size_t mini_capacity, double downsize_load_factor, double upsize_load_factor, double sweep_factor)
{
    ...
    gc->paused = false;
    gc->bos = bos;
    initial_capacity = initial_capacity < mini_capacity ? mini_capacity : initial_capacity;
    gc->allocs = gc_allocation_map_new(mini_capacity, initial_capacity, sweep_factor, downsize_limit, upsize_limit);
    LOG_DEBUG("Create new garbage collector (cap=%ld, siz=%ld)", gc->allocs->capacity, gc->allocs->size);
}
...
void gc_mark_alloc(GarbageCollector *gc, void *ptr)
{
    ...
    if (alloc && !(alloc->tag & GC_TAG_MARK))
    {
        ...
        {
            LOG_DEBUG("Checking allocation (ptr=%p) @%lu with value %p", ptr, p - ((char *)alloc->ptr), *(void **)p);
